...
def load_sleap_file(analysisfile=None):
    ...
    with h5py.File(analysisfile, 'r') as af:
        dataset = np.array(af["tracks"][
                           :].T)  # loads and transposes core tracking data. Dataset has four dimensions: frames, nodes, coordinates and instances (num. of animals)exi
